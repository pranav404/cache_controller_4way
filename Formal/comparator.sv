module comparator_selector(
    input logic [0:3][19:0] in_tags,  // Array containing 4 20-bit tags, one tag for each cache way
    input logic [19:0] in_cmp_tag,   //tag from CPU's requested address
    input logic [0:3][511:0] in_data, // data in cache line in current set
    input logic [0:3] valid_in , //Valid bits that say if a cache line is valid or not
...
`ifdef FORMAL
    // assertion to check correct working of way_hit 1

    initial assume (valid_in==4'b000);
    always@(*)begin
        assume (in_tags[0]!=in_tags[1]);
        assume (in_tags[0]!=in_tags[2]);
        assume (in_tags[0]!=in_tags[3]);
        assume (in_tags[1]!=in_tags[0]);
        assume (in_tags[1]!=in_tags[2]);
        assume (in_tags[1]!=in_tags[3]);
        assume (in_tags[2]!=in_tags[1]);
        assume (in_tags[2]!=in_tags[3]);
        assume (in_tags[2]!=in_tags[0]);
        assume (in_tags[3]!=in_tags[1]);
        assume (in_tags[3]!=in_tags[2]);
        assume (in_tags[3]!=in_tags[0]); // 2 or more tags cannot be the same
    end

    always@(*) begin
        if(in_tags[0]!=0 && in_cmp_tag!=0)begin
            if(in_tags[0]==in_cmp_tag && valid_in[0])
                assert(way_hit1);
        end
    end
    // assertion to check correct working of way_hit 2
    always@(*) begin
        if(in_tags[1]!=0 && in_cmp_tag!=0)begin
            if(in_tags[1]==in_cmp_tag && valid_in[1])
                assert(way_hit2);
        end
    end
    // assertion to check correct working of way_hit 3
    always@(*) begin
        if(in_tags[2]!=0 && in_cmp_tag!=0)begin
            if(in_tags[2]==in_cmp_tag && valid_in[2])
                assert(way_hit3);
        end
    end
    // assertion to check correct working of way_hit 4
    always@(*) begin
        if(in_tags[3]!=0 && in_cmp_tag!=0)begin
            if(in_tags[3]==in_cmp_tag && valid_in[3])
                assert(way_hit4);
        end
    end

    // assertions to check correct working of way_hit 1 procedure
    always@(*) begin
        if(way_hit1)begin
           assert(hit_data==in_data[0] && matched_way == 2'b00);
        end
    end

    // assertions to check correct working of way_hit 2 procedure
    always@(*) begin
        if(way_hit2)begin
           assert(hit_data==in_data[1] && matched_way == 2'b01);
        end
    end
    ...
    always@(*) begin
        if(way_hit3)begin
           assert(hit_data==in_data[2] && matched_way == 2'b10);
        end
    end

    // assertions to check correct working of way_hit 3 procedure
    always@(*) begin
        if(way_hit4)begin
           assert(hit_data==in_data[3] && matched_way == 2'b11);
        end
    end

    // assertions to check correct working no hit procedure
    always@(*) begin
        if(way_hit1==0 && way_hit2==0 && way_hit3 ==0 & way_hit4==0)begin
           assert(hit_data==0 && matched_way == 0);
        end
    end
